...
async function verifyForexData() {
  ...
  try {
    ...
    // Group by currency
    const byCurrency = allRecords.reduce(
      (acc, record) => {
    ...
    for (const [currency, records] of Object.entries(byCurrency)) {
      ...
      recentRecords.forEach((record, index) => {
        const createdAt = DateTime.fromJSDate(record.createdAt).setZone(
          "Africa/Cairo"
        );
        ...
        console.log(
          `   Spread: ${spread.toFixed(4)} (${((spread / record.open) * 100).toFixed(2)}%)`
        );
      });
    ...
    for (const [currency, records] of Object.entries(byCurrency)) {
      ...
      console.log(`\n${currency}:`);
      console.log(
        `  Status: ${isActive ? "🟢 Active (< 1 hour old)" : "🔴 Inactive (> 1 hour old)"}`
      );
      console.log(
        `  Latest update: ${DateTime.fromJSDate(latest.createdAt).toRelative()}`
      );
      console.log(
        `  Current predicted close: ${latest.predictedClose?.toFixed(4) || "N/A"}`
      );
    }
  } catch (error) {
