...
export function startForexSchedule() {
  ...
  cron.schedule("* * * * *", async () => {
    ...
    console.log(
      `Forex Scheduler: Checking if should run at ${now.toFormat("yyyy-MM-dd HH:mm:ss")} Cairo time`
    );
 ...
 */
async function shouldRunForexUpdate(
  now: DateTime
  ...
  if (weekday === 5 || weekday === 6) {
    return {
      run: false,
      reason: `Weekend (${weekday === 5 ? "Friday" : "Saturday"})`
    };
  }
  ...
  try {
    ...
    console.warn(
      "Forex Scheduler: Failed to check holiday status, proceeding anyway"
    );
  }
  ...
  if (timeInMinutes < startTime || timeInMinutes > endTime) {
    return {
      run: false,
      reason: `Outside trading hours (current: ${now.toFormat("HH:mm")}, trading: 09:50-14:20)`
    };
  }
...
async function runForexUpdate(now: DateTime) {
  ...
  for (const currency of ["USD", "GBP"] as const) {
    ...
    try {
      ...
      console.log(
        `Forex Scheduler: Got ${currency}/EGP rate: ${priceData.rate}`
      );
      ...
      if (!todaysRecord) {
        ...
        console.log(
          `Forex Scheduler: First request of the day for ${currency} - setting OPEN price to ${priceData.rate}`
        );
        ...
        if (
          ...
          console.log(
            `Forex Scheduler: Skipping ${currency} - values unchanged from last record (open=${lastSavedRecord.open}, high=${lastSavedRecord.high}, low=${lastSavedRecord.low})`
          );
          skippedCount++;
          continue;
        }
        ...
        console.log(
          `Forex Scheduler: Created new ${currency} record with OPEN=${priceData.rate} (this will remain fixed for the day)`
        );
      } else {
        ...
        console.log(
          `Forex Scheduler: Comparing ${currency} values - Current: open=${currentOpen}, high=${todaysRecord.high}, low=${todaysRecord.low} | New rate: ${priceData.rate} | Would result in: open=${currentOpen}, high=${newHigh}, low=${newLow}`
        );
        ...
        if (
          ...
          console.log(
            `Forex Scheduler: Skipping ${currency} - no changes needed (open=${currentOpen}, high=${newHigh}, low=${newLow})`
          );
          skippedCount++;
          continue;
        }
        ...
        console.log(
          `Forex Scheduler: Updating existing ${currency} record (OPEN=${currentOpen} remains fixed)`
        );
        const updateResult = await service.updateExistingPriceRecord(
          todaysRecord,
          priceData
        );
        console.log(
          `Forex Scheduler: ${currency} update result: ${updateResult}`
        );
      }
      ...
      console.error(
        `Forex Scheduler: Failed to update ${currency}:`,
        error.message
      );
    }
  }
  ...
  console.log(
    `Forex Scheduler: Update completed - ${successCount}/${totalCount} currencies updated, ${skippedCount} skipped (no changes)`
  );
}
...
async function checkAndRunOnStartup() {
  const now = DateTime.now().setZone("Africa/Cairo");
  console.log(
    `Forex Scheduler: Startup check at ${now.toFormat("yyyy-MM-dd HH:mm:ss")} Cairo time`
  );
  ...
  if (shouldRun.run) {
    ...
    console.log(
      `Forex Scheduler: Skipping startup update - ${shouldRun.reason}`
    );
  }
}
