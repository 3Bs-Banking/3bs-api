...
export class PersonalInvestmentRecommendationService extends BaseService<PersonalInvestmentRecommendation> {
  ...
  async create(
    ...
    console.log(
      `[InvestmentRecommendation] Processing submission for customer ${data.customer.id}`
    );
    ...
    if (isDuplicate) {
      console.log(
        `[InvestmentRecommendation] Duplicate submission detected for customer ${data.customer.id}`
      );
      throw new Error(
        "You have already submitted a questionnaire with these exact same preferences. Please modify your risk level or investment capacity if you want to submit again."
      );
    }

    console.log(
      `[InvestmentRecommendation] Creating new submission for customer ${data.customer.id}`
    );
   ...
   */
  private async deletePreviousRecommendations(
    customerId: string
  ): Promise<void> {
    console.log(
      `[InvestmentRecommendation] Checking for previous recommendations to delete for customer ${customerId}`
    );
    ...
    if (existingRecommendations.length > 0) {
      console.log(
        `[InvestmentRecommendation] Found ${existingRecommendations.length} previous recommendations to delete`
      );
      ...
      console.log(
        `[InvestmentRecommendation] Successfully deleted ${existingRecommendations.length} previous recommendations`
      );
    } else {
      console.log(
        `[InvestmentRecommendation] No previous recommendations found for customer ${customerId}`
      );
    }
  }
   ...
   */
  private async updateCustomerQuestionnaireFlag(
    customerId: string
  ): Promise<void> {
    console.log(
      `[InvestmentRecommendation] Updating questionnaireFilled flag for customer ${customerId}`
    );
    ...
    if (customer) {
      ...
      console.log(
        `[InvestmentRecommendation] Successfully updated questionnaireFilled flag to 1 for customer ${customerId}`
      );
    } else {
      console.warn(
        `[InvestmentRecommendation] Customer ${customerId} not found when updating questionnaireFilled flag`
      );
    }
  }
  ...
  private async checkForDuplicate(
    customerId: string,
    newInputData: ModelInput
  ): Promise<boolean> {
    console.log(
      `[InvestmentRecommendation] Checking for duplicate submission for customer ${customerId}`
    );
    ...
    for (const submission of existingSubmissions) {
      if (this.isSameData(submission.inputData, newInputData)) {
        console.log(
          `[InvestmentRecommendation] Found duplicate submission with ID: ${submission.id}`
        );
        return true;
      }
    }
    ...
    console.log(
      "[InvestmentRecommendation] No duplicate found, submission is unique"
    );
    return false;
  }
  ...
  private isSameData(existingData: any, newData: ModelInput): boolean {
    ...
    const sameCapacity =
      existingData.investmentCapacity === newData.investmentCapacity;
    const sameTransactionType =
      existingData.transactionType === newData.transactionType;

    return (
      sameRiskLevel && sameCustomerType && sameCapacity && sameTransactionType
    );
  }
  ...
  private async createNewSubmission(
    ...
    const { riskLevel, customerType, investmentCapacity, transactionType } =
      data.inputData;
    ...
    console.log(
      `[InvestmentRecommendation] Creating submission for customer ${data.customer.id}:`
    );
    console.log(`[InvestmentRecommendation] - Risk Level: ${riskLevel}`);
    console.log(
      `[InvestmentRecommendation] - Customer Type: ${customerType} (auto-classified)`
    );
    console.log(
      `[InvestmentRecommendation] - Investment Capacity: ${investmentCapacity} EGP`
    );
    console.log(
      `[InvestmentRecommendation] - Transaction Type: ${transactionType}`
    );
    ...
    console.log(
      `[InvestmentRecommendation] Successfully created submission with ${outputData.length} recommendations for customer ${data.customer.id}`
    );
  ...
  async findAllByCustomerId(
    ...
    console.log(
      `[InvestmentRecommendation] Finding all submissions for customer: ${customerId}`
    );
  ...
  async findLatestByCustomerId(
    customerId: string
  ): Promise<PersonalInvestmentRecommendation | null> {
    console.log(
      `[InvestmentRecommendation] Finding latest submission for customer: ${customerId}`
    );
    ...
    if (latest) {
      console.log(
        `[InvestmentRecommendation] Found latest submission for customer ${customerId}: ${latest.id}`
      );
    } else {
      console.log(
        `[InvestmentRecommendation] No submissions found for customer: ${customerId}`
      );
    }
  ...
  private async generateRecommendations(
    ...
    console.log(
      "[InvestmentRecommendation] Sending classified data to ML model:"
    );
    console.log(
      `[InvestmentRecommendation] - Customer Type: ${modelInput.customerType}`
    );
    console.log(
      `[InvestmentRecommendation] - Risk Level: ${modelInput.riskLevel}`
    );
    console.log(
      `[InvestmentRecommendation] - Investment Capacity: ${modelInput.investmentCapacity} EGP`
    );
    console.log(
      `[InvestmentRecommendation] - Transaction Type: ${modelInput.transactionType}`
    );
  ...
  private async callMLModel(
    ...
    console.log(
      `[InvestmentRecommendation] Calling ML model with classified customer type: ${modelInput.customerType}`
    );
  ...
  private generateMockRecommendations(
    ...
    const { riskLevel, customerType, investmentCapacity } = modelInput;

    console.log(
      `[InvestmentRecommendation] Mock ML Model processing classified customer: ${customerType}`
    );

    const recommendations = this.getRecommendationsByCustomerType(
      customerType,
      investmentCapacity,
      riskLevel
    );
    ...
    console.log(
      `[InvestmentRecommendation] Mock ML Model generated ${recommendations.length} recommendations for ${customerType}`
    );
  ...
  private getRecommendationsByCustomerType(
    ...
    console.log(
      `[InvestmentRecommendation] ML Model processing customer classification: ${customerType}`
    );
    ...
    switch (customerType) {
      ...
      default:
        console.warn(
          `[InvestmentRecommendation] Unknown customer type: ${customerType}, using Mass strategy`
        );
        strategy = {
          assetTypes: ["Investment Fund", "Stock", "Bond"],
          sectors: [
    ...
    console.log(
      `[InvestmentRecommendation] Using strategy for ${customerType}: ROI ${strategy.roiRange.min}-${strategy.roiRange.max}%`
    );

    // Generate 5 varied ROI values in descending order (highest first)
    const roiValues = this.generateDescendingROIValues(
      strategy.roiRange.min,
      strategy.roiRange.max
    );
    ...
    const recommendations = Array.from({ length: 5 }, (_, i) => {
      return {
        ...
        Industry: this.getIndustryBySector(
          strategy.sectors[i % strategy.sectors.length]
        ),
        "ROI (%)": roiValues[i],
    ...
    console.log(
      "[InvestmentRecommendation] Sorted recommendations by ROI (highest first):"
    );
    recommendations.forEach((rec, index) => {
      console.log(
        `[InvestmentRecommendation] ${index + 1}. ISIN: ${rec.ISIN}, ROI: ${rec["ROI (%)"]}%, AssetType: ${rec.AssetType}`
      );
    });
    ...
    if (recommendations.length > 0) {
      ...
      console.log(
        `[InvestmentRecommendation] Set highest ROI recommendation as recommended: ISIN ${recommendations[0].ISIN} with ${recommendations[0]["ROI (%)"]}% ROI (1st position)`
      );
    }
  ...
  private adjustStrategyByRiskLevel(
    ...
    if (risk === "low" || risk === "conservative") {
      ...
      strategy.roiRange.max = Math.max(
        strategy.roiRange.max - 8,
        strategy.roiRange.min + 15
      );
