...
app.get("/:temporaryAccess", (req, res) =>
  tempAccessController.getId(req, res)
);
...
app.patch("/:temporaryAccess", (req, res) =>
  tempAccessController.update(req, res)
);

// Delete a temporary access record (manual removal, if needed)
app.delete("/:temporaryAccess", (req, res) =>
  tempAccessController.delete(req, res)
);

// Custom endpoint: Check if a user exists (for employee lookup in frontend)
app.get("/check-user/:id", (req, res) =>
  tempAccessController.checkUser(req, res)
);
